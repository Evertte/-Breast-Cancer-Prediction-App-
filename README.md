...
A Streamlit-powered web application (https://bcancerpredictionapp.streamlit.app/) that predicts whether a breast tumor is malignant or benign using machine learning. The model is trained on the **Breast Cancer dataset available via Seaborn**, which includes features extracted from digitized images of fine needle aspirates (FNA) of breast masses.
